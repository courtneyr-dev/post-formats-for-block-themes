...
- Lightbox Slideshow gallery variation (`blocks/gallery-variations/lightbox-slideshow/`): each `.wp-block-image` in a `.is-style-lightbox-slideshow` gallery now gets a real `<button class="pfbt-lightbox-trigger" aria-label="Open {alt text or 'image N'} in lightbox">` appended to it, instead of the figure itself being made focusable via `tabindex="0"` plus a manual click/keydown handler. The lightbox dialog is now a native `<dialog>` opened with `.showModal()` — Escape-to-close and Tab focus cycling come from the browser instead of a hand-rolled keydown handler, and focus returns to the button that opened the dialog (not just the figure) when it closes.
- Status format pattern (`patterns/status.php`): removed the hardcoded `"fontSize":"large"` block attribute from the status paragraph. Size now comes from a theme-paintable CSS custom property, `--pfbt-format-status-font-size` (defaults to `inherit`), applied via `.pfbt-format-status .status-paragraph` in `styles/format-tokens.css`.
- Removed trailing empty `<p></p>` placeholder paragraphs seeded by the `aside`, `audio`, `chat`, `gallery`, `image`, `link`, `quote`, `standard`, and `video` format patterns. An unfilled placeholder rendered as an empty paragraph on the front end and was announced as a blank line by screen readers. A new front-end-only `render_block_core/paragraph` filter (`PFBT_A11y_Output`, skipped when `is_admin()`) also strips any paragraph block that renders as empty, so a future empty paragraph doesn't ship to visitors either way. The filter matches the equivalent one already shipped in the companion courtneyr-child theme (`inc/a11y-output.php`) so both hold consistently.
- Link format pattern (`patterns/link.php`): the fallback markup's empty anchor (`<a href="#"></a>`) is now a plain placeholder paragraph with no anchor element, so an unfilled fallback no longer ships a link that goes nowhere.
- Title-less formats (Aside, Quote, Status) now carry a visually-hidden `post-title` block — `level 1` on the single templates (`single-format-aside.html`, `single-format-status.html`), `level 2` in the archive/single display patterns (`patterns/display/aside.php`, `patterns/display/quote.php`, `patterns/display/status.php`) — so every post has a real heading for screen readers and document outline, even though the title stays visually hidden. The existing visually-hidden rule in `styles/format-tokens.css` now also targets the new `.pfbt-format-title-sr` class.
- Chat Log block (`blocks/chatlog/`): the Discord device-frame timestamp color changed from `#72767d` to `#b9bbbe` against the `#36393f` background, raising contrast from 2.54:1 to roughly 6:1 and meeting WCAG AA. The block's outer container's `role="log"` changed to `role="region"`, since its content isn't a live log of new messages arriving.
- Status display pattern (`patterns/display/status.php`): the archive/single author avatar is now marked decorative (`alt=""`, `role="presentation"`) via a `render_block_core/avatar` filter scoped to that pattern's `pfbt-status-avatar` block className, since the adjacent author-name link already conveys the same information. The filter does not affect avatars anywhere else (comments, other patterns, theme templates).
- Filter Tags gallery variation (`blocks/gallery-variations/filter-tags/view.js`): the chip bar's `role="toolbar"` (which implies arrow-key roving-tabindex navigation the code never implemented) changed to `role="group"`; the chips already expose their state via `aria-pressed` and are independently focusable via Tab.
- Lookbook Hotspots gallery variation (`blocks/gallery-variations/lookbook-hotspots/view.js`): pressing Escape while a hotspot button is focused now dismisses its open popover.
- Before/After Pairs gallery variation (`blocks/gallery-variations/before-after-pairs/view.js`): fixed a bug where pairing two images cloned only their `<img>` elements, silently dropping any `<figcaption>` text from the DOM. The whole figure (image plus caption) is now cloned for both the before and after sides.

## [1.1.7] - 2026-09-22

### Security

- `execute_mf2_markup()`, `execute_mf2_validate()`, `execute_posse_prepare()`, and `execute_get_post_format()` each validated a sitewide capability (`read` or `edit_posts`) in their ability's `permission_callback`, then loaded whatever `post_id` the caller supplied with a bare `get_post()` — no check that the caller could act on that specific post. A Subscriber could read a private post's content through `post-formats/mf2-markup`, and a Contributor could run `post-formats/posse-prepare` against another user's draft, by passing that post's ID. Added `pfbt_ability_post_or_error()` (`includes/abilities/functions-ability-guards.php`), a shared guard every post-bound ability now runs its `post_id` through: it 404s on a missing, invalid (`$post_id < 1`), or non-`post` post as `pfbt_not_found` (was `invalid_post` at each individual call site), then checks `read_post`/`edit_post` against that specific post before returning it, as `pfbt_forbidden`.
- The same guard closes a second gap in its `read_post` branch: `current_user_can( 'read_post', $id )` maps to the primitive `read` capability for any published post, with no awareness of a post password — so any logged-in user could read a password-protected post's content through `mf2-markup`. The guard now also requires `post_password_required( $post )` to be false, or the caller to hold `edit_post` on it. `post_password_required()` itself has no such exemption; the analogous rule already exists in core as `WP_REST_Posts_Controller::can_access_password_content()`, which grants password-protected content to anyone who holds `edit_post` when the request context is `edit`.
- `post-formats/get-format-signals` registered with `permission_callback => '__return_true'`. Changed to `array( $this, 'can_read_posts' )`, the same method the other three MCP abilities already use, rather than a duplicate inline closure.
...
- Deactivating and reactivating the plugin no longer restores a synced pattern the site owner hand-deleted, since pattern registration is now gated on `pfbt_version` rather than a transient that deactivation cleared unconditionally (see the transient-vs-version behavior described under Changed below). `pfbt_deactivate()` now also clears `pfbt_version`, so the next `admin_init` after reactivation re-registers any missing pattern the same way 1.1.6's deactivation hook did.

### Changed

- Synced-pattern creation (`PFBT_Pattern_Manager::register_all_patterns()`) previously ran on the `init` hook, on every admin, admin-ajax, and REST request, gated by a one-week transient (`pfbt_patterns_registered`) rather than by `PFBT_VERSION`. That meant it re-ran roughly every week regardless of whether the plugin version had changed — and each re-run overwrote any admin edit to a synced pattern's content back to the plugin's shipped default, since `create_synced_pattern()` replaces `post_content` whenever it differs from the pattern file. Patterns are now (re)created only in `pfbt_maybe_upgrade()`, on the first `admin_init` request after `PFBT_VERSION` changes — once per version, not once a week — so upgrading to 1.1.7 overwrites a hand-edited pattern once, and leaves it alone until the next version bump. Activation no longer writes `pfbt_version` itself (it only sets `pfbt_activated_time`); `pfbt_maybe_upgrade()` records the version only once the patterns it corresponds to have actually been (re)created, so a context where `register_all_patterns()` no-ops — WP-CLI, Playground, or `admin_init` fired outside `is_admin()` — doesn't strand the site on a version number with no matching patterns. `pfbt_register_patterns()` remains as a deprecated wrapper, guarded against a missing `PFBT_Pattern_Manager` class, so third-party code still calling it by name does not fatal.
