...
- `pfbt_activate()` called `PFBT_Pattern_Manager::force_register_patterns()` directly. An activation hook fires via an `include_once()` of the plugin's main file at the exact moment WordPress activates it, which can happen before that same request's own `plugins_loaded` has fired `pfbt_include_files()` again — so `PFBT_Pattern_Manager` was not guaranteed to be loaded, and activation could fatal with "Class PFBT_Pattern_Manager not found". Activation no longer references that class or records `pfbt_version` at all; both now happen only in `pfbt_maybe_upgrade()` on `admin_init`, once the class is loaded and `is_admin()` is true.

### Changed

- Synced-pattern creation (the `wp_block` post inserts behind `PFBT_Pattern_Manager::register_all_patterns()`) moved off the `init` hook, where it ran on every admin, admin-ajax, and REST request — and, once per version bump, wrote to the database — regardless of who was asking. Patterns are now (re)created only in `pfbt_maybe_upgrade()`, on the first `admin_init` after `PFBT_VERSION` changes, which covers both a fresh activation and an in-place upgrade. `pfbt_register_patterns()` remains as a deprecated wrapper so third-party code still calling it by name does not fatal.
