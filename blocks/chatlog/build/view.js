...
document.addEventListener("DOMContentLoaded", function () {
  // Skip if Interactivity API is handling this block.
  if (document.querySelector(".chatlog[data-wp-interactive]")) {
    return;
  }

  /**
   * Thread toggle functionality
   *
  ...
  threadToggles.forEach(function (toggle) {
    ...
    toggle.addEventListener("click", function () {
      ...
      if (thread) {
        ...
        this.setAttribute(
          "aria-expanded",
          thread.classList.contains("chatlog-thread--collapsed")
            ? "false"
            : "true",
        );
      }
