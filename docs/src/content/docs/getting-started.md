...
description: "Create your first formatted post and learn the three ways a post gets its format: the selection modal, the Format Switcher, and auto-detection."
---

Create your first formatted post and learn the three ways a post gets its format: the modal, the Format Switcher, and auto-detection. These steps assume the plugin is active on a block theme — see [Installation](/post-formats-for-block-themes/installation/) if it isn't yet.
...
- **Formats mostly inherit your theme's styling.** The plugin ships structural tokens, not a design; your theme's colors and typography apply. To restyle a format, use the Site Editor — see the [Site Editor guide](https://github.com/courtneyr-dev/post-formats-for-block-themes/blob/main/SITE-EDITOR-GUIDE.md) and the [format customization quick start](https://github.com/courtneyr-dev/post-formats-for-block-themes/blob/main/FORMAT-CUSTOMIZATION-QUICK-START.md).
- **The first block is locked on purpose.** It preserves the format's structure. Add more blocks after it.
- **Older versions (before 1.1.5) had known editor issues** — the modal could stack over the welcome guide, and Quote/Video patterns could show block validation errors. Update to the latest version if you see these; see [Troubleshooting](/post-formats-for-block-themes/troubleshooting/).

## Where to go next

- Try the **Chat Log block**: see [Common tasks](/post-formats-for-block-themes/common-tasks/) for publishing a chat transcript.
- Review [Settings](/post-formats-for-block-themes/settings/) for the icon set picker and the repair tool.
