...
<div class="wrap">
	...
	if ( $pfbt_repair_message ) {
		...
		?>
		<div class="notice <?php echo esc_attr( $pfbt_notice_class ); ?> is-dismissible">
			<p><strong><?php echo esc_html( $pfbt_repair_message['message'] ); ?></strong></p>
		</div>
		<?php
	...
	<div class="pfbt-template-section" style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #ccc;">
		<h2><?php esc_html_e( 'Template Assignments', 'post-formats-for-block-themes' ); ?></h2>

		<div class="pfbt-repair-summary card">
			<h3><?php esc_html_e( 'Template Scan Results', 'post-formats-for-block-themes' ); ?></h3>

			<table class="widefat striped">
				<tbody>
					<tr>
						<th scope="row"><?php esc_html_e( 'Total Posts Scanned:', 'post-formats-for-block-themes' ); ?></th>
						<td><strong><?php echo esc_html( number_format_i18n( $template_results['total_scanned'] ) ); ?></strong></td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Correct Templates:', 'post-formats-for-block-themes' ); ?></th>
						<td><span class="dashicons dashicons-yes-alt" style="color: #46b450;" aria-hidden="true"></span> <?php echo esc_html( number_format_i18n( $template_results['correct'] ) ); ?></td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Incorrect Templates:', 'post-formats-for-block-themes' ); ?></th>
						<td>
							<?php if ( $template_results['issue_count'] > 0 ) : ?>
								<span class="dashicons dashicons-warning" style="color: #f0b849;" aria-hidden="true"></span>
								<strong><?php echo esc_html( number_format_i18n( $template_results['issue_count'] ) ); ?></strong>
							<?php else : ?>
								<span class="dashicons dashicons-yes-alt" style="color: #46b450;" aria-hidden="true"></span>
								<?php esc_html_e( 'None', 'post-formats-for-block-themes' ); ?>
							<?php endif; ?>
		...
		<?php if ( $template_results['issue_count'] > 0 ) : ?>
			<div class="pfbt-template-actions" style="margin-top: 20px;">
				<h3><?php esc_html_e( 'Fix Template Assignments', 'post-formats-for-block-themes' ); ?></h3>

				<p class="description">
					<?php esc_html_e( 'The following posts have incorrect template assignments. This can happen after plugin updates or changes to the template system.', 'post-formats-for-block-themes' ); ?>
				</p>

				<form method="post" action="" id="pfbt-fix-templates-form">
					<?php wp_nonce_field( 'pfbt_repair_action', 'pfbt_repair_nonce' ); ?>
					<input type="hidden" name="pfbt_repair_action" value="fix_all_templates" />

					<p>
						<button type="submit" class="button button-primary button-large">
							<span class="dashicons dashicons-admin-tools" aria-hidden="true" style="margin-top: 3px;"></span>
							<?php esc_html_e( 'Fix All Template Assignments', 'post-formats-for-block-themes' ); ?>
						</button>
					</p>
				</form>
			</div>

			<div class="pfbt-template-issues" style="margin-top: 30px;">
				<h3><?php esc_html_e( 'Template Issues', 'post-formats-for-block-themes' ); ?></h3>

				<table class="wp-list-table widefat striped table-view-list">
					<thead>
						<tr>
							<th scope="col"><?php esc_html_e( 'Post Title', 'post-formats-for-block-themes' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Format', 'post-formats-for-block-themes' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Current Template', 'post-formats-for-block-themes' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Expected Template', 'post-formats-for-block-themes' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Action', 'post-formats-for-block-themes' ); ?></th>
						</tr>
					</thead>
					<tbody>
						<?php foreach ( $template_results['issues'] as $pfbt_issue ) : ?>
							<tr>
								<td>
									<strong>
									...
									<span class="description">
										<?php
										/* translators: %d: Post ID */
										echo esc_html( sprintf( __( 'ID: %d', 'post-formats-for-block-themes' ), $pfbt_issue['post_id'] ) );
										?>
									</span>
								</td>
								<td>
									<code><?php echo esc_html( ucfirst( $pfbt_issue['format'] ) ); ?></code>
								</td>
								<td>
									<code style="color: #d63638;"><?php echo esc_html( $pfbt_issue['current_template'] ); ?></code>
								</td>
								<td>
									<code style="color: #00a32a;"><?php echo esc_html( $pfbt_issue['expected_template'] ); ?></code>
								</td>
								<td>
									<form method="post" action="" style="display: inline;">
										<?php wp_nonce_field( 'pfbt_repair_action', 'pfbt_repair_nonce' ); ?>
										<input type="hidden" name="pfbt_repair_action" value="fix_single_template" />
										<input type="hidden" name="post_id" value="<?php echo esc_attr( $pfbt_issue['post_id'] ); ?>" />
										<input type="hidden" name="format" value="<?php echo esc_attr( $pfbt_issue['format'] ); ?>" />
										<button type="submit" class="button button-small">
											<?php esc_html_e( 'Fix', 'post-formats-for-block-themes' ); ?>
										</button>
									</form>
								</td>
							</tr>
		...
		<?php else : ?>
			<div class="notice notice-success inline" style="margin-top: 20px;">
				<p>
					<span class="dashicons dashicons-yes-alt" aria-hidden="true"></span>
					<strong><?php esc_html_e( 'Perfect!', 'post-formats-for-block-themes' ); ?></strong>
					<?php esc_html_e( 'All your posts have the correct template assignments.', 'post-formats-for-block-themes' ); ?>
				</p>
			</div>
		<?php endif; ?>
	</div>
