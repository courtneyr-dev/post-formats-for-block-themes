...
* Changed: the ten format patterns are now (re)created at plugin activation and, for in-place upgrades, the first time an admin visits wp-admin after the version changes — not from an `init` hook that ran the check, and once per version the database write, on every admin, admin-ajax, or REST request regardless of who was asking. Activation itself no longer creates patterns or records the plugin version directly, since its hook can fire before the plugin's own classes are guaranteed loaded; a `pfbt_register_patterns()` function remains as a deprecated no-op-safe wrapper for any third-party code still calling it by name.
